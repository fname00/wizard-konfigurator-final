(function($){
  ...
  $('#cele-list').on('click','.cel',function(){
    ...
    $.each(groups,function(type,list){
      ...
      list.forEach(function(f){
        ul.append('<li><label><input type="checkbox" data-price="'+f.price+'" value="'+f.title+'"> '+f.title+'</label></li>');
      });
  ...
  $('#next-2').click(function(){
    ...
    selectedFeatures.forEach(function(f){
      $('#summary-list').append('<label><input type="checkbox" class="sum-item" data-price="'+f.price+'" value="'+f.title+'" checked> '+f.title+' ('+f.price+' zł)</label>');
    });
  ...
  $('#finish').click(function(){
    ...
    var feats=$('#summary-list .sum-item:checked').map(function(){return $(this).val();}).get().join(',');
    save({features:feats, budget:$('#budget').val(), email: email}, function(){
